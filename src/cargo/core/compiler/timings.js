...
function render_pipeline_graph() {
  ...
  const { ctx, graph_width, canvas_width, canvas_height, px_per_sec } =
    draw_graph_axes("pipeline-graph", graph_height);
  ...
  for (i = 0; i < units.length; i++) {
    ...
    const labelName = (unitCount.get(unit.name) || 0) > 1
...
function render_timing_graph() {
  ...
  const { canvas_width, graph_width, ctx } = draw_graph_axes(
    "timing-graph",
    AXIS_HEIGHT,
  );
  ...
  const { step, tick_dist, num_ticks } = split_ticks(
    max_v,
    px_per_v,
    GRAPH_HEIGHT,
  );
  ...
  draw_line("blue", function (c) {
    return c.inactive;
  });
  draw_line("red", function (c) {
    return c.waiting;
  });
  draw_line("green", function (c) {
    return c.active;
  });
...
function pipeline_mouse_hit(event) {
  ...
  for (let box of HIT_BOXES) {
    if (
      event.offsetX >= box.x && event.offsetX <= box.x2 &&
